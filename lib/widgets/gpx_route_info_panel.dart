...
/// GPX 路線資訊面板：從底部延伸、圓弧頂、拖曳線、X 關閉（類似步道資訊面板）
class GpxRouteInfoPanel extends StatefulWidget {
  final GpxRouteInfo info;
  ...
  @override
  State<GpxRouteInfoPanel> createState() => _GpxRouteInfoPanelState();
}

class _GpxRouteInfoPanelState extends State<GpxRouteInfoPanel> {
  ...
  Widget _buildChart() {
    final data = widget.info.chartData;
    ...
    final yInterval = range > 0 ? range / 2 : 1.0; // 固定 4 個刻度
              ...
              LineChartData(
                ...
                lineTouchData: LineTouchData(
                  ...
                  touchCallback: (event, response) {
                    ...
                    var best = data.first;
                    var bestDiff =
                        ((best['distance'] as num).toDouble() - touchedDist)
                            .abs();
                    for (final p in data) {
                      final d =
                          ((p['distance'] as num).toDouble() - touchedDist)
                              .abs();
                      if (d < bestDiff) {
                        bestDiff = d;
                        best = p;
                    ...
                    if (best['lat'] != null && best['lon'] != null) {
                      cb(LatLng(
                        (best['lat'] as num).toDouble(),
                        (best['lon'] as num).toDouble(),
                      ));
                    } else {
                      cb(null);
                    }
